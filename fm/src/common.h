    ...
    bool const do_update=false)
{
    ...
    __m128 const XMMones = _mm_set1_ps(1.0f);
    ...
    for(uint32_t f1 = 0; f1 < nr_field; ++f1)
    {
        ...
        for(uint32_t f2 = f1; f2 < nr_field; ++f2)
        {
            ...
            if(do_update)
            {
                ...
                for(uint32_t d = 0; d < nr_factor; d += 4)
                {
                    if( f1 != f2 ){
                        ...
                        __m128 XMMw1 = _mm_load_ps(w1+d);
                        __m128 XMMwg1 = _mm_load_ps(wg1+d);
                        __m128 XMMg1 = _mm_add_ps(_mm_mul_ps(XMMlambda, XMMw1),_mm_mul_ps(XMMkappav, XMMones));
                        XMMwg1 = _mm_add_ps(XMMwg1, _mm_mul_ps(XMMg1, XMMg1));
            ...
            else
            {
                for(uint32_t d = 0; d < nr_factor; d += 4)
                {
                    ...
                    if( f1 != f2 ){
                        ...
                        XMMt = _mm_add_ps(XMMt,_mm_mul_ps(_mm_mul_ps(XMMw1, XMMones), XMMv));
                    }
                }
            }
        }
